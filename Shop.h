...
class Shop{
 public:
  ...
  // Preconditions - Requires default values (Name = Test, numIngredients = 0, \
Rank = F                                                                        
  // Postconditions - Creates a new Player Shop                                 
  Shop();
  // Name: Shop(name) - Overloaded constructor                                  
  // Desc - Used to build a new Player Shop (m_numIngredients = 0, m_shopRank =\
 F)                                                                             
  ...
  // Postconditions - Add a ingredient to the Shop's m_myIngredients with a qua\
ntity of 0                                                                      
...
// Desc - Reduces quantity from the Shop's inventory with true, if no quantit\
y false                                                                         
 ...
 ingredient)                                                                    
  ...
  // Desc - Loops through m_myIngredients to see how many ingredients have a qu\
antity > 0                                                                      
  // Preconditions - The shop exists, m_myIngredients populated                 
  // Postconditions - Returns the count of ingredients with a quantity > 0      
  int IngredientCount();
// Name: ShopScore()                                                          
  // Desc - Displays information about the shop: name, ingredients found, total\
 available,                                                                     
  //        percentage found, and rank                                          
  // Preconditions - The shop exists, m_myIngredients populated                 
  // Postconditions - Displays name, ingredient count, total ingredients, perce\
ntage, and rank                                                                 
  void ShopScore();
private:
  string m_shopName; //Name of the Shop                                         
  char m_shopRank; // Rank of the Shop, can be (D, C, B, or A) (When 'S', game \
is over)                                                                        
  int m_numIngredients; //Current number of items in m_myIngredients known to t\
he Player Shop                                                                  
  Ingredient m_myIngredients[PROJ2_SIZE] = {}; //List of ingredients known to t\
his Shop                                                                        
};
